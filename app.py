...
st.markdown("Confronta i prezzi dei prodotti di ReadyPro con i prezzi aggiornati di Amazon")
...
# Carica il file Keepa (Excel)
keepa_file = st.sidebar.file_uploader("Carica file Keepa (Excel)", type=["xlsx"])
...
if ready_pro_file is not None and keepa_file is not None:
    ...
    ### Rinominazione delle colonne nel file ReadyPro
    # Il file ReadyPro deve contenere le colonne:
    # SKU, Sito, Codice(ASIN), Descrizione sul marketplace, Q.aggiornata, Prz.sito
    ...
    ### Parsing del prezzo nel file ReadyPro
    try:
    ...
    # Il file Keepa deve contenere la colonna "Buy Box: Current" che usiamo come Prezzo di riferimento
    if "Buy Box: Current" in df_keepa.columns:
        df_keepa["Prezzo di riferimento"] = df_keepa["Buy Box: Current"].apply(parse_price)
    else:
        st.error("Il file Keepa deve contenere la colonna 'Buy Box: Current'.")
    ...
    if "ASIN" not in df_keepa.columns:
        ...
        ### Colonne da visualizzare: includiamo quelle utili per l'aggiornamento da ReadyPro
    ...
    st.info("Attendere il caricamento dei file ReadyPro e Keepa.")
